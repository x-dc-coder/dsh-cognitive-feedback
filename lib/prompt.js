...
export function renderCognitiveSection(state, options = {}) {
  ...
  if (active.kind === 'teaching_back') {
    return [
      '[COGNITIVE FEEDBACK]',
      // `reason` here is a topic key derived from user text. It is sanitized
      // to at most six lowercase [a-z0-9-] words by topicKey(), which is the
      // injection boundary; it is never raw user input.
      active.reason
