...
export function activeIntervention(state) {
  if (state.pendingGate) {
    return { kind: 'gate', reason: state.lastGateTopic ?? state.taskType ?? 'decision' };
  }
  if (
    ...
    return { kind: 'challenge', reason: state.taskType ?? 'debugging' };
  }
  if (state.teachingBackPending) {
    return { kind: 'teaching_back', reason: state.completedHighValueTopic ?? state.taskType ?? 'task' };
  }
  return null;
}
