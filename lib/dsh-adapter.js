...
export function installAdapter(ctx, config = {}) {
   ...
   */
  const guarded = (label, callback) => (...args) => {
    try {
      callback(...args);
    } catch (error) {
  ...
  try {
    disposers.push(
      ctx.on(
        'session/created',
        guarded('session/created', (session) => {
  ...
  try {
    disposers.push(
      ctx.on(
        'session/disposed',
        guarded('session/disposed', (session) => {
          const sessionId = String(session?.id ?? session);
  ...
  try {
    disposers.push(
      ctx.on(
        'session/event',
        guarded('session/event', (session, event) => {
          const sessionId = String(session?.id ?? session);
  ...
  try {
    ctx.inject(['systemPrompt'], (scoped) => {
      ...
      // the calling fiber. The callback below runs when that service is present;
      // when it is absent the fiber simply stays pending and the plugin continues
      // to observe and log events without prompt injection. Injection is
      // therefore optional in effect, though the dependency itself is declared.
      //
